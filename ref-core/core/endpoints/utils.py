...
class Build:
    ...
    def plot_raw(self):
        ...
        plt.axis([0, 1.2*self.ref.sizes['pressure']['max'], 0, 1.2*self.ref.sizes['uptake']['max']])
        plt.xlabel('Pressure (Bar)')
        plt.ylabel('Uptake (mmol/g)')
        plt.grid(b=True, which='major', color='k', linestyle='-')
        # plt.legend(legend, loc = 2, fontsize = 10)
    ...
    def plot_error_bar(self):
        ...
        plt.plot(self.ref.fit_pressure, self.ref.md_uptake, '^', ms = float(5.0), color = "red", mew = .25, ls = '-', lw = float(1.5), zorder = 3)
        plt.fill_between(np.array(self.ref.fit_pressure), 1.04*self.a_max, 0.96*(2*np.array(self.ref.md_uptake)-self.a_max), facecolor="red", alpha=0.5)
...
class Evaluation:
    ...
    def run(self):
        ...
        if len(self.pressure['aliq1']['run1']) >0:
            ref1_a1 = np.interp(fine_grid, self.pressure['aliq1']['run1'], self.uptake['aliq1']['run1'], left=None, right=None, period=None)
            U.append(ref1_a1)
        if len(self.pressure['aliq1']['run2']) >0:
            ref2_a1 = np.interp(fine_grid, self.pressure['aliq1']['run2'], self.uptake['aliq1']['run2'], left=None, right=None, period=None)
            U.append(ref2_a1)

        # a2_U.append(self.ref.mn_uptake['aliq2'])
        if len(self.pressure['aliq2']['run1']) >0:
            ref1_a2 = np.interp(fine_grid, self.pressure['aliq2']['run1'], self.uptake['aliq2']['run1'], left=None, right=None, period=None)
            U.append(ref1_a2)
        if len(self.pressure['aliq2']['run2']) >0:
            ref2_a2 = np.interp(fine_grid, self.pressure['aliq2']['run2'], self.uptake['aliq2']['run2'], left=None, right=None, period=None)
            U.append(ref2_a2)

        for _set in self.sets:
            info = _set
            if len(info['raw-pressure']['aliq1']['run1']) >0:
                ref1_a1 = np.interp(fine_grid, info['raw-pressure']['aliq1']['run1'], info['raw-uptake']['aliq1']['run1'], left=None, right=None, period=None)
                U.append(ref1_a1)
            if len(info['raw-pressure']['aliq1']['run2']) >0:
                ref1_a2 = np.interp(fine_grid, info['raw-pressure']['aliq1']['run2'], info['raw-uptake']['aliq1']['run2'], left=None, right=None, period=None)
                U.append(ref1_a2)

            if len(info['raw-pressure']['aliq2']['run1']) >0:
                ref2_a1 = np.interp(fine_grid, info['raw-pressure']['aliq2']['run1'], info['raw-uptake']['aliq2']['run1'], left=None, right=None, period=None)
                U.append(ref2_a1)
            if len(info['raw-pressure']['aliq2']['run2']) >0:
                ref2_a2 = np.interp(fine_grid, info['raw-pressure']['aliq2']['run2'], info['raw-uptake']['aliq2']['run2'], left=None, right=None, period=None)
                U.append(ref2_a2)
        ...
        self.results['fit-pressure'] = fine_grid.tolist()
    ...
    def plot_error(self):
        ...
        plt.plot(self.pressure['aliq1']['run1'], self.uptake['aliq1']['run1'], '^', ms = float(5.0), color = "yellow", mew = .25, ls = '-', lw = float(1.5), zorder = 3)
        plt.plot(self.pressure['aliq1']['run2'], self.uptake['aliq1']['run2'], 'v', ms = float(5.0), color = "yellow", mew = .25, ls = '-', lw = float(1.5), zorder = 3)
        plt.plot(self.pressure['aliq2']['run1'], self.uptake['aliq2']['run1'], '<', ms = float(5.0), color = "yellow", mew = .25, ls = '-', lw = float(1.5), zorder = 3)
        plt.plot(self.pressure['aliq2']['run2'], self.uptake['aliq2']['run2'], '>', ms = float(5.0), color = "yellow", mew = .25, ls = '-', lw = float(1.5), zorder = 3)

        plt.fill_between(self.results['fit-pressure'], 1.04*self.a_max, 0.96*(2*np.array(self.results['md'])-self.a_max), facecolor="green", alpha=0.5)
        plt.plot(self.results['fit-pressure'], self.results['md'], 'o', ms = float(5.0), color = "green", mew = .25, ls = '-', lw = float(1.5), zorder = 3)
    ...
    def error(self):
        ...
        if not self.agrees[0]:
            agree = "disagreed"
        self.plot_error()
        ref_path = 'plots/tmp-error-ref-{0}-{1}-{2}.png'.format(str(self.ref.id), self.eval_id.split('.')[0], agree)
        print ref_path
        plt.savefig(ref_path)
        plt.close()
        return ref_path
